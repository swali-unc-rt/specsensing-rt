...
def create_graph_from_csv(csv_filepath):
    ...
    # 5. Format the graph
    plt.xlabel('Channel Count', fontsize=22)
    # Updated label to reflect the conversion
    plt.ylabel('Response Time (µs)', fontsize=22) 
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)

    plt.title('Optimized Batching vs Uniform Batching', fontsize=22)
    ...
    plt.xlim(left=df['channelcount'].min()*0.99, right=max_x * 1.01)
